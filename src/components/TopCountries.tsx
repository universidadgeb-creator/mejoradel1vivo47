...
export default function TopCountries({
  ...
  return (
    <ol className="flex flex-col gap-2">
      {countries.map((c, i) => (
        <li key={`${c.club}-${c.pais}`}>
          <button
            type="button"
            onClick={() => onSelect?.(c.club, c.pais)}
            className="flex w-full items-center gap-3 rounded-lg text-left hover:bg-neutral-50 dark:hover:bg-neutral-800"
          >
