...
export default function StreaksList({
  ...
  return (
    <ol className="flex flex-col gap-2">
      {streaks.map((s, i) => (
        <li key={`${s.club}-${s.pais}`}>
          <button
            type="button"
            onClick={() => onSelect?.(s.club, s.pais)}
            className="flex w-full items-center gap-3 rounded-xl border border-black/5 p-3 text-left hover:bg-neutral-50 dark:border-white/10 dark:hover:bg-neutral-800"
          >
